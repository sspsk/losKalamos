...
@bp.route('/vlavor/addregion', methods=('POST', ))
def addregion():
    db = get_db()
    cur = db.cursor()
...
@bp.route('/vlavor/addarea', methods=('POST', ))
def addarea():
    db = get_db()
    cur = db.cursor()
...
@bp.route('/vlavor/getareas',methods = ('GET',))
def getareas():
...
@bp.route('/vlavor/register', methods=('POST', ))
def register():
...
@bp.route('/vlavor/civil',methods=('GET',))
def civil_index():
    db = get_db()
    cur = db.cursor(cursor_factory = psycopg2.extras.DictCursor)
...
@bp.route('/vlavor/', methods=('GET','POST'))
def index():
    db = get_db()
    cur = db.cursor(cursor_factory = psycopg2.extras.DictCursor)
...
@bp.route('/vlavor/delRegion',methods = ('POST',))
def delRegion():
    region = request.form['region']
    print(region)
    db = get_db()
...
@bp.route('/vlavor/delArea', methods=('POST',))
def delArea():
    area = request.form['area']
    db = get_db()
    cur = db.cursor(cursor_factory = psycopg2.extras.DictCursor)
